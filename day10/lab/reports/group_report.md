...
| Rủi ro còn lại | Mức độ | Tác động | Việc chưa làm / hướng xử lý |
|---|---|---|---|
| Lỗi embedding hoặc ChromaDB khi publish | Cao | Không cập nhật được vector mới, Agent có thể trả lời theo dữ liệu cũ | Thêm retry cho embed + alert khi upsert thất bại |
| Freshness FAIL chưa có cảnh báo tự động | Cao | Phát hiện trễ khi dữ liệu nguồn stale | Tích hợp webhook Slack/Email cho `freshness_check=FAIL` |
| `chunk_id` còn phụ thuộc `seq` | Trung bình | Có nguy cơ prune nhầm khi thứ tự dữ liệu thay đổi | Rà soát chiến lược id ổn định hơn, bổ sung checksum |
| Chưa benchmark ở quy mô lớn | Trung bình | Chưa rõ giới hạn hiệu năng khi tăng số lượng chunk | Chạy benchmark với dataset >100 chunks và ghi lại throughput |
| LLM-judge chủ yếu dựa top-1 | Thấp | Có thể bỏ sót lỗi nằm ở top-k | Mở rộng đánh giá theo nhiều chunk (top-k) trong vòng sau |

**Ưu tiên triển khai tiếp theo:**
- **Trong 24h:** hoàn thiện alert cho embed/freshness (2 mục mức cao).
- **Trong 1 tuần:** giảm rủi ro `chunk_id`, tăng giám sát expectation halt.
- **Trong 1 sprint:** benchmark quy mô lớn và nâng cấp LLM-judge theo top-k.
